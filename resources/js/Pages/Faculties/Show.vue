<template>
    <AuthenticatedLayout>
        <template #header>{{ faculty.name }}</template>
        ...
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button @click="deleteTarget = faculty"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">{{ faculty.name }}</h2>
                            <p class="text-sm text-gray-600 mt-1">{{ faculty.code }}</p>
                        </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs text-gray-500">Dekan</p>
                                    <p class="text-sm font-semibold text-gray-900 truncate">
                                        {{ faculty.dean?.name || '—' }}
                                    </p>
                                    <p v-if="faculty.dean?.email" class="text-xs text-gray-400 truncate">
                                        {{ faculty.dean.email }}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <p class="text-xs text-gray-500">Yaratilgan sana</p>
                                    <p class="text-sm font-semibold text-gray-900">{{ formatDate(faculty.created_at) }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <p class="text-xs text-gray-500">Kafedralar</p>
                                        <p class="text-sm font-semibold text-gray-900">{{ departments_count }} ta</p>
                                    </div>
                                </div>
                                <Link :href="`/departments?faculty_id=${faculty.id}`"
                    ...
                    <!-- Tavsif -->
                    <div v-if="faculty.description" class="mt-4 p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                        <p class="text-xs font-medium text-indigo-700 mb-1">Tavsif</p>
                        <p class="text-sm text-indigo-800">{{ faculty.description }}</p>
                    </div>
                </div>
            </div>
...
const props = defineProps({
    faculty:           Object,
    departments_count: { type: Number, default: 0 },
...
function doDelete() {
    ...
    router.delete(`/faculties/${props.faculty.id}`, {
        onSuccess: () => {
            ...
            else { toast.success('Fakultet muvaffaqiyatli o\'chirildi!'); router.visit('/faculties') }
        },
